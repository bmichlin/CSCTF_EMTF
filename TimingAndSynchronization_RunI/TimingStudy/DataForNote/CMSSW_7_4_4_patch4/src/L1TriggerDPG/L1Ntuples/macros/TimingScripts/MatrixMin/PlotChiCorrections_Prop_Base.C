...
void Matrix_Sync(TString filename, int runNumber, int station, int ring, const int numChambersInRing, int endcap, double* inputChiValues, double* inputChiErrors){
    ...
    for(int i = 1; i<numChambersInRing+1; i++){
        ...
        TString waRelBX = TString::Format("WalkAroundProp_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,endcap);
...
ofstream writeFile(TString::Format("Correction_Table_Prop_%d_ME%d_%d%d.txt",runNumber, endcap, station, ring),ios::app);
	cout << "+-----------------------------------------------+" << endl;
...
void ChiSquarePlots(TString filename, int runNumber, int station, int ring, int numChambersInRing, int endcap){
   ...
   vector<double>  clbg_x; //clbg=CSCTF LCT BX Graph with Chi Correction
	...
	for(int x = 1; x < numChambersInRing+1; x++){
		...
		TString chiIndBX_low = TString::Format("WalkAroundProp_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,station,ring,lowChamber,endcap);
		TString chiIndBX_high = TString::Format("WalkAroundProp_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,station,ring,highChamber,endcap);
		...
		TString chiRelBX = TString::Format("WalkAroundProp_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,endcap);
		...
		TString bxHist = TString("BX_Prop_")+cap+TString::Format("ME_%d%d_%d_run%d",station,ring,x,runNumber);
...
for(int k=1;k<numChambersInRing;k++) h->GetXaxis()->SetBinLabel(k,xlabel[k-1]);
	...
	chi_corr_graph->Draw("AP");
       ...
       chi_RelBX_graph_fit->SetTitle(TString("Relative BX with Signal Propagation and Chi Corrections for ME")+sign+Form("%d%d",station,ring));
        ...
        graph->SetTitle(TString("CSCTF LCT BX With Signal Propagation and Chi Correction ME")+sign+Form("%d%d",station,ring));
	...
	mycanvas5->SaveAs(cap+TString::Format("Matrix_WalkAround_Chi_Prop_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas6->SaveAs(cap+TString::Format("Matrix_Chi_Timing_Correction_Prop_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas9->SaveAs(cap+TString::Format("Matrix_WalkAround_RelBX_withChi_Prop_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas7->SaveAs(cap+TString::Format("Matrix_BX_By_Chamber_Chi_and_prop_ME%d%d_%d.pdf",station,ring,runNumber));
	//mycanvas8->SaveAs(TString::Format("BX_By_Chamber_TOF_Chi_ME%d%d_%d.pdf",station,ring,runNumber));
