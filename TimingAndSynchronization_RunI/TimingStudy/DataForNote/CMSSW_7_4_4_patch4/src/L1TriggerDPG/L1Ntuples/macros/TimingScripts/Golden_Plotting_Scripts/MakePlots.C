...
void MakePlots(TString filename, int runNumber, int station, int ring, int numChambersInRing, int endcap){
	ofstream writeFile(TString::Format("Number_of_Entries_%d_ME%d_%d%d.txt",runNumber, endcap, station, ring),ios::app);
	writeFile << "+-------------------------------------------------------+\n";
	writeFile << "|\tOverlap Region\t|\tNumber of CSCTF LCTs\t|\n";
	...
	for(int x = 1; x < numChambersInRing+1; x++){
		...
		writeFile << "|\t" << lowChamber << " - " << highChamber  << "\t\t|\t\t" << getHist(tfile,waRelBX)->GetEntries() << "\t\t|\n";
...
TGraphErrors* wa_RelBX_graph_fit = new TGraphErrors(numChambersInRing,array_WA_Rel_BX_x,array_WA_Rel_BX_y,array_WA_Rel_BX_ex,array_WA_Rel_BX_ey);
...
cout << "" << endl;
	...
	wa_RelBX_graph_fit->SetTitle(TString("Relative BX for ME")+sign+Form("%d%d",station,ring));
	wa_RelBX_graph_fit->SetMarkerColor(8);
	wa_RelBX_graph_fit->SetMarkerStyle(21);
	wa_RelBX_graph_fit->SetMarkerSize(1);
	wa_RelBX_graph_fit->GetXaxis()->SetTitle("Chamber Number");
	wa_RelBX_graph_fit->GetYaxis()->SetTitle("BX (25ns)");
	TCanvas* mycanvas3 = new TCanvas();
	mycanvas3->SetGrid();
	TAxis *axis = wa_RelBX_graph_fit->GetYaxis();
	axis->SetRangeUser(-4,4);

	wa_RelBX_graph_fit->Draw("AP");
	gStyle->SetOptFit(0001);
	wa_RelBX_graph_fit->Fit("pol0","F");
	wa_RelBX_graph_fit->GetFunction("pol0")->SetLineColor(8);
	...
	mycanvas->SaveAs(cap+TString::Format("BX_By_Chamber_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas1->SaveAs(cap+TString::Format("WalkAround_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas2->SaveAs(cap+TString::Format("BX_By_Chamber_TOF_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas3->SaveAs(cap+TString::Format("WalkAround_relBX_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas4->SaveAs(cap+TString::Format("WalkAround_High_Low_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas10->SaveAs(cap+TString::Format("IncidentAngle_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas11->SaveAs(cap+TString::Format("IncidentAngleNieghbors_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas12->SaveAs(cap+TString::Format("XCoordinate_By_Chamber_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas13->SaveAs(cap+TString::Format("YCoordinate_By_Chamber_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas14->SaveAs(cap+TString::Format("ZCoordinate_By_Chamber_ME%d%d_%d.pdf",station,ring,runNumber));
