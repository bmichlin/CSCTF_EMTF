...
void Matrix_Sync(TString filename, int runNumber, int station, int ring, const int numChambersInRing, int endcap, double* inputChiValues, double* inputChiErrors){
    ...
    for(int i = 1; i<numChambersInRing+1; i++){
        ...
        TString waRelBX = TString::Format("WalkAroundwithTOF_And_Prop_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,endcap);
...
ofstream writeFile(TString::Format("Correction_Table_TOFChi_prop_%d_ME%d_%d%d.txt",runNumber, endcap, station, ring),ios::app);
	cout << "+-----------------------------------------------+" << endl;
...
vector<double> chi_on_tof_x;
vector<double> chi_on_tof_y;
vector<double> chi_on_tof_ex;
vector<double> chi_on_tof_ey;
	...
	for(int x = 1; x < numChambersInRing+1; x++){
		...
		TString chiIndBX_low = TString::Format("WalkAroundwithTOF_And_Prop_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,station,ring,lowChamber,endcap);
		TString chiIndBX_high = TString::Format("WalkAroundwithTOF_And_Prop_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,station,ring,highChamber,endcap);
		...
		TString chiRelBX = TString::Format("WalkAroundwithTOF_And_Prop_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,endcap);
...
/*
//This plot is the CSCTF LCT BX with Chi Correction.  Doesn't make sense to make this graph for _TOFCHI
...
*/
		...
		TString bxTOF = TString("TOF_And_PropforNeighbors_BX_")+cap+TString::Format("_ME%d%d_%d",station,ring,x);
		...
		TString bxTOF2 = TString("TOF_BX_")+cap+TString::Format("_ME%d%d_%d",station,ring,x);

		chi_on_tof_x.push_back(x);
		chi_on_tof_y.push_back((getHist(tfile,bxTOF2)->GetMean()) + inputChiValues[lowChamber-1]);
		chi_on_tof_ex.push_back( (0.5) );
		chi_on_tof_ey.push_back(sqrt( (getHist(tfile,bxTOF2)->GetMeanError()*getHist(tfile,bxTOF2)->GetMeanError()) + (inputChiErrors[lowChamber-1]*inputChiErrors[lowChamber-1])));
...
double* array_chi_on_tof_x = &chi_on_tof_x[0];
double* array_chi_on_tof_y = &chi_on_tof_y[0];
double* array_chi_on_tof_ex= &chi_on_tof_ex[0];
double* array_chi_on_tof_ey= &chi_on_tof_ey[0];
	...
	//TGraphErrors* chi_on_tof_graph = new TGraphErrors(numChambersInRing,array_chi_on_tof_x,array_chi_on_tof_y,array_chi_on_tof_ex,array_chi_on_tof_ey);

	//Stylistic Choices for Plots (and Plotting)	

	TString plotTitle = TString("BX (with ChiSquare and TOF and Signal Propagation Corrections) for ME")+sign+Form("%d%d",station,ring);
...
for(int k=1;k<numChambersInRing;k++) h->GetXaxis()->SetBinLabel(k,xlabel[k-1]);
	...
	chi_corr_graph->SetTitle(TString("Suggested Timing Correction (with Signal Propagation) to ME")+sign+Form("%d%d",station,ring));
...
/*
	...
	graph->Draw("AP");
*/
	tof_bx_graph->SetTitle(TString("CSCTF LCT BX With TOF and Signal Propagation and Chi Corrections ME")+sign+Form("%d%d",station,ring));
	...
	//
	//chi_on_tof_graph->SetTitle(TString("CSCTF LCT BX With Chi on TOF Corrections ME")+sign+Form("%d%d",station,ring));
	//chi_on_tof_graph->SetMarkerColor(2);
	//chi_on_tof_graph->SetMarkerStyle(21);
	//chi_on_tof_graph->SetMarkerSize(1);
	//chi_on_tof_graph->GetXaxis()->SetTitle("Chamber Number");
	//chi_on_tof_graph->GetYaxis()->SetTitle("BX (25ns)");
	//TCanvas* mycanvas9 = new TCanvas();
	//mycanvas9->SetGrid();
	//TAxis *axis2 = chi_on_tof_graph->GetYaxis();
	//axis2->SetRangeUser(4,8);
	//chi_on_tof_graph->Draw("AP");
	//gStyle->SetOptFit(0001);
	//chi_on_tof_graph->Fit("pol0","F"); //The F option uses the minuit fitter.  Adding Q ("FQ") makes the fit not print.
	...
	mycanvas5->SaveAs(cap+TString::Format("Matrix_WalkAround_Chi_TOFChi_prop_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas6->SaveAs(cap+TString::Format("Matrix_Chi_Timing_Correction_TOFChi_prop_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas7->SaveAs(cap+TString::Format("Matrix_WalkAround_RelBX_withChi_TOFChi_prop_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas8->SaveAs(cap+TString::Format("Matrix_BX_By_Chamber_TOF_Chi_TOFChi_prop_ME%d%d_%d.pdf",station,ring,runNumber));
	//mycanvas9->SaveAs(TString::Format("BX_By_Chamber_Chi_On_TOF_TOFChi_ME%d%d_%d.pdf",station,ring,runNumber));
	printTable(inputChiValues, inputChiErrors, runNumber, endcap, station, ring);
	//mycanvas7->SaveAs("BX_By_Chamber_Chi_TOFChi.pdf");
}
