...
void Matrix_Sync(TString filename, int runNumber, int station, int ring, const int numChambersInRing, int endcap, double* inputChiValues, double* inputChiErrors){
    ...
    for(int i = 1; i<numChambersInRing+1; i++){
        ...
        TString waRelBX = TString::Format("WalkAroundwithTOF_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,endcap);
...
ofstream writeFile(TString::Format("Correction_Table_TOFChi_%d_ME%d_%d%d.txt",runNumber, endcap, station, ring),ios::app);
	cout << "+-----------------------------------------------+" << endl;
...
vector<double> chi_on_tof_ey;
	...
	for(int x = 1; x < numChambersInRing+1; x++){
		...
		TString chiIndBX_low = TString::Format("WalkAroundwithTOF_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,station,ring,lowChamber,endcap);
		TString chiIndBX_high = TString::Format("WalkAroundwithTOF_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,station,ring,highChamber,endcap);
		...
		TString chiRelBX = TString::Format("WalkAroundwithTOF_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,endcap);
...
*/
		...
		TString bxTOF = TString("TOFforNeighbors_BX_")+cap+TString::Format("_ME%d%d_%d",station,ring,x);
...
double* array_chi_on_tof_ey= &chi_on_tof_ey[0];
	...
	TString plotTitle = TString("BX (with ChiSquare and TOF Correction) for ME")+sign+Form("%d%d",station,ring);
...
*/
	...
	mycanvas5->SaveAs(cap+TString::Format("Matrix_WalkAround_Chi_TOFChi_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas6->SaveAs(cap+TString::Format("Matrix_Chi_Timing_Correction_TOFChi_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas7->SaveAs(cap+TString::Format("Matrix_WalkAround_RelBX_withChi_TOFChi_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas8->SaveAs(cap+TString::Format("Matrix_BX_By_Chamber_TOF_Chi_TOFChi_ME%d%d_%d.pdf",station,ring,runNumber));
	mycanvas9->SaveAs(cap+TString::Format("Matrix_BX_By_Chamber_Chi_On_TOF_TOFChi_ME%d%d_%d.pdf",station,ring,runNumber));
