...
void MakeChiSquare(TString filename, int runNumber, int station, int ring, int numChambersInRing, int endcap){
	...
	ofstream writeFile("chiSquare.txt",ios::app);
	writeFile << "double chisquare =\n";
	for(int chamber = 1; chamber < 37; chamber++){
		...
		if(chamber != numChambersInRing){
			...
			writeFile << "pow(  ( " << Form("x%d",chamber-1) << " - " << Form("x%d",chamber) << " + " << getHist(tfile, waRelBX)->GetMean() << " )  / " << getHist(tfile, waRelBX)->GetMeanError() << " ,2)  +  \n";
		}else{
			TString waRelBX = TString::Format("WalkAroundwithTOF_And_Prop_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,endcap);
			cout << "pow(  ( " << Form("x%d",chamber-1) << " - " << "x0" << " + " << getHist(tfile, waRelBX)->GetMean() << " )  / " << getHist(tfile, waRelBX)->GetMeanError() << " ,2);  " << endl; 
			writeFile << "pow(  ( " << Form("x%d",chamber-1) << " - " << "x0" << " + " << getHist(tfile, waRelBX)->GetMean() << " )  / " << getHist(tfile, waRelBX)->GetMeanError() << " ,2)  +  \n";
writeFile << "0; \n"; 	
		}
