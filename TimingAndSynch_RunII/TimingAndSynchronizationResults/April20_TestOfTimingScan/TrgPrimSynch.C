...
vector<int> revector_int(vector<int>* vec){
	vector<int> newVector;
	for(int i = 0; i < vec->size(); i++){
		int inter = vec->at(i);
		newVector.push_back(inter);
	}
	return newVector;
}

vector<float> revector_float(vector<float>* vec){
	//cout << "Checkpoint 1" << endl;
	vector<float> newVector;
	//cout << "Checkpoint 2" << endl;
	for(int i = 0; i < vec->size(); i++){
		//cout << "Checkpoint 3 (in loop)" << endl;
		float floater = vec->at(i);
		//cout << "Checkpoint 4 (in loop)" << endl;
		newVector.push_back(floater);
		//cout << "Checkpoint 5 (in loop)" << endl;
	}
	//cout << "Checkpoint 1" << endl;
	return newVector;
}
...
/*}}}*/
void run(TString file, int runNumber, Long64_t numEvents, int EventSelection, int stationInput, int ringInput, int numChambersInRing, int ME2Trigger, int ME3Trigger, int forwardOnly, int PileUp50ns) 
{
cout <<"Begin" << endl;
TFile *myfile = new TFile(file);
myfile->cd("ntuple");	
TTree *t = (TTree*)myfile->Get("ntuple/tree");
//cout<<"  Events  "<<t->GetEntries()<<endl;
/*
Int_t numLCTs;
vector<float>* lctGlobalPhi;
vector<float>* lctGlobalEta;
vector<int>*   lctLocPhi;
vector<int>*   lctLocEta;
...
t->SetBranchAddress("lctGlobalPhi",    &lctGlobalPhi   );
t->SetBranchAddress("lctGlobalEta",    &lctGlobalEta   );
t->SetBranchAddress("lctLocPhi",       &lctLocPhi      );
t->SetBranchAddress("lctLocEta",       &lctLocEta      );
t->SetBranchAddress("lctEndcap",       &lctEndcap      );
t->SetBranchAddress("lctSector",       &lctSector      );
t->SetBranchAddress("lctSubSector",    &lctSubSector   );
t->SetBranchAddress("lctBx",           &lctBx          );
t->SetBranchAddress("lctBx0",          &lctBx0         );
t->SetBranchAddress("lctStation",      &lctStation     );
t->SetBranchAddress("lctRing",         &lctRing        );
t->SetBranchAddress("lctChamber",      &lctChamber     );
t->SetBranchAddress("lctTriggerCSCID", &lctTriggerCSCID);
t->SetBranchAddress("lctStrip",        &lctStrip       );
t->SetBranchAddress("lctWire",         &lctWire        );
*/

Int_t numLCTs;
vector<float>* vlctGlobalPhi = 0;
vector<float>* vlctGlobalEta = 0;
vector<int>*   vlctLocPhi = 0;
vector<int>*   vlctLocEta = 0;
vector<int>*   vlctEndcap = 0;
vector<int>*   vlctSector = 0;
vector<int>*   vlctSubSector = 0;
vector<int>*   vlctBx = 0;
vector<int>*   vlctBx0 = 0;
vector<int>*   vlctStation = 0;
vector<int>*   vlctRing = 0;
vector<int>*   vlctChamber = 0;
vector<int>*   vlctTriggerCSCID = 0;
vector<int>*   vlctStrip = 0;
vector<int>*   vlctWire = 0;

//cout << "Declared vector pointers" << endl;
gROOT->ProcessLine("#include <vector>");
t->SetBranchAddress("lctGlobalPhi",    &vlctGlobalPhi   );
t->SetBranchAddress("lctGlobalEta",    &vlctGlobalEta   );
t->SetBranchAddress("lctLocPhi",       &vlctLocPhi      );
t->SetBranchAddress("lctLocEta",       &vlctLocEta      );
t->SetBranchAddress("lctEndcap",       &vlctEndcap      );
t->SetBranchAddress("lctSector",       &vlctSector      );
t->SetBranchAddress("lctSubSector",    &vlctSubSector   );
t->SetBranchAddress("lctBx",           &vlctBx          );
t->SetBranchAddress("lctBx0",          &vlctBx0         );
t->SetBranchAddress("lctStation",      &vlctStation     );
t->SetBranchAddress("lctRing",         &vlctRing        );
t->SetBranchAddress("lctChamber",      &vlctChamber     );
t->SetBranchAddress("lctTriggerCSCID", &vlctTriggerCSCID);
t->SetBranchAddress("lctStrip",        &vlctStrip       );
t->SetBranchAddress("lctWire",         &vlctWire        );

//cout << "Filled vector pointers" << endl;
/*
vector<float> lctGlobalPhi   = revector_float(vlctGlobalPhi   ); 
vector<float> lctGlobalEta   = revector_float(vlctGlobalEta   ); 
vector<int> lctLocPhi      = revector_int(vlctLocPhi      );
vector<int> lctLocEta      = revector_int(vlctLocEta      );
vector<int> lctEndcap      = revector_int(vlctEndcap      );
vector<int> lctSector      = revector_int(vlctSector      ); 
vector<int> lctSubSector   = revector_int(vlctSubSector   );
vector<int> lctBx          = revector_int(vlctBx          );
vector<int> lctBx0         = revector_int(vlctBx0         );
vector<int> lctStation     = revector_int(vlctStation     );
vector<int> lctRing        = revector_int(vlctRing        );
vector<int> lctChamber     = revector_int(vlctChamber     );
vector<int> lctTriggerCSCID= revector_int(vlctTriggerCSCID);
vector<int> lctStrip       = revector_int(vlctStrip       );
vector<int> lctWire        = revector_int(vlctWire        );
...
////cout << "runNumber: " << runNumber << endl;	
	...
	if((numEvents == -1 && eventCounter < t->GetEntries()) || (numEvents != -1 && eventCounter < numEvents)){
		if(numEvents == -1) numEvents = t->GetEntries();						
...
//cout << "Revector successful" << endl;
	...
	//		cout << "Event Number: " << e << endl;
			...
			for(int i = 0; i < lctBx.size(); i++){
				//cout << "LCT Index " << i << " (Station, Ring, Chamber, GlobalEta, GlobalPhi, BX, Endcap): " << endl;
				//cout <<  lctStation[i] << ", " << lctRing[i] << ", " << lctChamber[i] << ", " << lctGlobalEta[i] << ", " << lctGlobalPhi[i] << ", " << lctBx[i] << ", " << lctEndcap[i] << endl;
				//cout << "" << endl;
			}
			...
			for(int i = 0; i < lctBx.size(); i++){
				...
				//////cout <<  lctStation[i] << ", " << lctRing[i] << ", " << lctChamber[i] << ", " << lctGlobalEta[i] << ", " << lctGlobalPhi[i] << ", " << lctBx[i] << endl;
				for(int j = 0; j < lctBx.size(); j ++){
					//Say that two LCTs are the same if they have the same station, ring, chamber, BX, eta, and phi.
					if(i != j){
						if(lctStation[i]   != lctStation[j]) continue;
						if(lctRing[i]      != lctRing[j]) continue;
						if(lctChamber[i]   != lctChamber[j]) continue;
						if(lctGlobalEta[i] != lctGlobalEta[j]) continue;
						if(lctGlobalPhi[i] != lctGlobalPhi[j]) continue;
						if(lctBx[i]        != lctBx[j]) continue;
						if(lctEndcap[i]        != lctEndcap[j]) continue;
						//////cout << "" << endl;
			...
			for(int i = 0; i < lctBx.size(); i++){
				...
				//cout <<  lctStation[i] << ", " << lctRing[i] << ", " << lctChamber[i] << ", " << lctGlobalEta[i] << ", " << lctGlobalPhi[i] << ", " << lctBx[i] << ", " << lctEndcap[i] << endl;
				if(lctStation[i] == 1 && lctRing[i] == 1 && lctEndcap[i] == 1) me11posCounter++;
				if(lctStation[i] == 1 && lctRing[i] == 1 && lctEndcap[i] == 2) me11negCounter++;			
			}

			//cout << "Checkpoint 3" << endl;
			NumLCTs->Fill(lctBx.size());
			...
			for(int j = 0; j < lctBx.size(); j ++){
				...
				//if(csctf_->lctQuality[j] <= 10) continue; //3/10/2016 Quality not part of new nTuple	

				if(lctStation[j] == stationInput && lctRing[j] == ringInput){
					statOne = true;
					statOneIndex.push_back(j);
					statOneChambers.push_back(lctChamber[j]);			
				}
				if(lctStation[j] == 2){
					statTwo = true;
			...
			for(int l = 0; l < lctBx.size(); l++){
				if(contains(duplicates, l) == true) continue;
				if(lctStation[l] != stationInput) continue;
				if(lctRing[l] != ringInput) continue;
				double xpos = getx(lctGlobalEta[l],lctGlobalPhi[l],stationInput,ringInput,lctSector[l]);
				double ypos = gety(lctGlobalEta[l],lctGlobalPhi[l],stationInput,ringInput,lctSector[l]);
				double zpos = getz(lctGlobalEta[l],lctGlobalPhi[l],stationInput,ringInput,lctEndcap[l], lctChamber[l]);
				if(lctEndcap[l] == 1){
					TString getcoordx = TString::Format("xCoordinate_pos_ME%d%d_%d",stationInput,ringInput,lctChamber[l]);
					TH1F *thexhist = ((TH1F *)(HListxCoordinate.FindObject(getcoordx)));
					thexhist->Fill(xpos);
					TString getcoordy = TString::Format("yCoordinate_pos_ME%d%d_%d",stationInput,ringInput,lctChamber[l]);
					TH1F *theyhist = ((TH1F *)(HListyCoordinate.FindObject(getcoordy)));
					theyhist->Fill(ypos);
					TString getcoordz = TString::Format("zCoordinate_pos_ME%d%d_%d",stationInput,ringInput,lctChamber[l]);
					TH1F *thezhist = ((TH1F *)(HListzCoordinate.FindObject(getcoordz)));
					thezhist->Fill(zpos);
				}else{
					TString getcoordx = TString::Format("xCoordinate_neg_ME%d%d_%d",stationInput,ringInput,lctChamber[l]);
					TH1F *thexhist = ((TH1F *)(HListxCoordinate.FindObject(getcoordx)));
					thexhist->Fill(xpos);
					TString getcoordy = TString::Format("yCoordinate_neg_ME%d%d_%d",stationInput,ringInput,lctChamber[l]);
					TH1F *theyhist = ((TH1F *)(HListyCoordinate.FindObject(getcoordy)));
					theyhist->Fill(ypos);
					TString getcoordz = TString::Format("zCoordinate_neg_ME%d%d_%d",stationInput,ringInput,lctChamber[l]);
					TH1F *thezhist = ((TH1F *)(HListzCoordinate.FindObject(getcoordz)));
					thezhist->Fill(zpos);
				}			
			}
			...
			for(int i =0; i < statOneIndex.size(); i++){
				if(lctEndcap[statOneIndex[i]] == 1) statOneIndexPos.push_back(statOneIndex[i]); 
				if(lctEndcap[statOneIndex[i]] == 2) statOneIndexNeg.push_back(statOneIndex[i]); 
			}
			for(int i =0; i < statTwoIndex.size(); i++){
				if(lctEndcap[statTwoIndex[i]] == 1) statTwoIndexPos.push_back(statTwoIndex[i]);
				if(lctEndcap[statTwoIndex[i]] == 2) statTwoIndexNeg.push_back(statTwoIndex[i]);
			}
			for(int i =0; i < statThreeIndex.size(); i++){
				if(lctEndcap[statThreeIndex[i]] == 1) statThreeIndexPos.push_back(statThreeIndex[i]);
...
//cout << "Checkpoint 2" << endl;
			...
			for(int i = 0; i < statOneIndex.size(); i++){
				...
				if(contains(statOneChambers,statOneChambers[i]+1)){
					if(lctEndcap[statOneIndex[findIndex(statOneChambers,statOneChambers[i])]] != lctEndcap[statOneIndex[findIndex(statOneChambers,statOneChambers[i]+1)]] )continue;	
				...
				if(statOneChambers[i] == numChambersInRing){
					if(contains(statOneChambers, 1)){
						if(lctEndcap[statOneIndex[findIndex(statOneChambers,numChambersInRing)]] != lctEndcap[statOneIndex[findIndex(statOneChambers,1)]] )continue;	
			...
			for(int n =0; n < neighborList.size(); n++){
				...
				//cout << "Neighbor Chambers Check: " << lctChamber[neighborList[n][0]] << ", " << lctChamber[neighborList[n][1]] << endl;
			}
...
//cout << "Duplicate cut passed" << endl;			
	//if(lctChamber[i] > numChambersInRing) ////cout << "Chamber Number out of range" << endl;
				//positive endcap only

				//if(csctf_->lctQuality[i] <= 10) continue; ////3/10/2016 Quality not part of new nTuple
//cout << "Quality cut passed" << endl;			
//cout << "endcap, station, ring: " << lctEndcap[i] <<  ", " << lctStation[i] << ", " << lctRing[i] << endl;
				if(lctEndcap[i] == 1){
//cout << "Positive endcap cut passed" << endl;				
	//cout << "Checkpoint 0" << endl;
					TString getWirePosHist = TString::Format("MEplus%d%d_WireGroup",lctStation[i],lctRing[i]);
					TH2F *theWirePosHist = ((TH2F *)(HListWire.FindObject(getWirePosHist)));
					theWirePosHist->Fill(lctChamber[i],lctWire[i]);

					if(lctStation[i] == stationInput && lctRing[i] == ringInput){
						TString gethist = TString::Format("Strip_BX_pos_ME%d%d_%d",(lctStation[i]),(lctRing[i]),(lctChamber[i]));
						TH2F *theHist = ((TH2F *)(HListStrip.FindObject(gethist)));
						theHist->Fill(lctStrip[i],lctBx[i]);
					}
					if(lctStation[i] == 1 && lctRing[i] == 1){
						ME11_pos_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 1 && lctRing[i] == 2){
						ME12_pos_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 1 && lctRing[i] == 3){
						ME13_pos_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 2 && lctRing[i] == 1){
						ME21_pos_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 2 && lctRing[i] == 2){
						ME22_pos_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 3 && lctRing[i] == 1){
						ME31_pos_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 3 && lctRing[i] == 2){
						ME32_pos_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 4 && lctRing[i] == 1){
						ME41_pos_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 4 && lctRing[i] == 2){
						ME42_pos_BX->Fill(lctBx[i]);
					}
				}
				//negative endcap only
				  //cout << "Checkpoint 3" << endl;
				if(lctEndcap[i] == 2){
					TString getWireNegHist = TString::Format("MEneg%d%d_WireGroup",lctStation[i],lctRing[i]);
					TH2F *theWireNegHist = ((TH2F *)(HListWire.FindObject(getWireNegHist)));
					theWireNegHist->Fill(lctChamber[i],lctWire[i]);					

					if(lctStation[i] == stationInput && lctRing[i] == ringInput){
						TString gethist = TString::Format("Strip_BX_neg_ME%d%d_%d",(lctStation[i]),(lctRing[i]),(lctChamber[i]));
						TH2F *theHist = ((TH2F *)(HListStrip.FindObject(gethist)));
						theHist->Fill(lctStrip[i],lctBx[i]);
					}
					if(lctStation[i] == 1 && lctRing[i] == 1){
						ME11_neg_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 1 && lctRing[i] == 2){
						ME12_neg_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 1 && lctRing[i] == 3){
						ME13_neg_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 2 && lctRing[i] == 1){
						ME21_neg_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 2 && lctRing[i] == 2){
						ME22_neg_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 3 && lctRing[i] == 1){
						ME31_neg_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 3 && lctRing[i] == 2){
						ME32_neg_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 4 && lctRing[i] == 1){
						ME41_neg_BX->Fill(lctBx[i]);
					}
					if(lctStation[i] == 4 && lctRing[i] == 2){
						ME42_neg_BX->Fill(lctBx[i]);
					}
				}
//cout << "Checkpoint 4" << endl;
				//BX by chamber 
				if(lctEndcap[i] == 2){
					TString gethist = TString::Format("BX_negME_%d%d_%d_run%d",(lctStation[i]),(lctRing[i]),(lctChamber[i]),(runNumber));
					TH1F *theHist = ((TH1F *)(HListneg.FindObject(gethist)));
					theHist->Fill(lctBx[i]);
				}else{ //pos endcap
					TString gethist = TString::Format("BX_posME_%d%d_%d_run%d",(lctStation[i]),(lctRing[i]),(lctChamber[i]),(runNumber));
...
//cout << "Checkpoint 6" << endl;
			...
			for(int n = 0; n < tempVector.size(); n++){
				if(lctEndcap[tempVector[n]] == 1){ 
					TString gethistprop = TString::Format("BX_Prop_posME_%d%d_%d_run%d",(lctStation[n]),(lctRing[n]),(lctChamber[n]),(runNumber));
					//	cout << "the next line is gethistprop" << endl; 
					//cout << gethistprop << endl;

					TH1F *theHistprop = ((TH1F *)(HList_prop.FindObject(gethistprop)));
					/*
					 * cout << "Ring (temp)                              : " << lctRing[tempVector[n]] << endl;
					 cout << "Ring (statOneIndex)                      : " << lctRing[statOneIndex[n]] << endl;
					 cout << "Station (temp)                           : " << lctStation[tempVector[n]] << endl;
					 cout << "Station (statOneIndex)                   : " << lctStation[statOneIndex[n]] << endl;
					 cout << "Strip (temp)                             : " << lctStrip[tempVector[n]] << endl;
					 cout << "Strip (statOneIndex)                     : " << lctStrip[statOneIndex[n]] << endl;
					 cout << "Wire (temp)                              : " << lctWire[tempVector[n]] << endl;
					 cout << "Wire (statOneIndex)                      : " << lctWire[statOneIndex[n]] << endl;
					 cout << "Ring, Station, Strip, Wire (temp)        : " << lctRing[tempVector[n]] << ", " << lctStation[tempVector[n]] << ", " << lctStrip[tempVector[n]] << ", " << lctWire[tempVector[n]] << endl;
					 cout << "Ring, Station, Strip, Wire (statOneIndex): " << lctRing[statOneIndex[n]] << ", " << lctStation[statOneIndex[n]] << ", " << lctStrip[statOneIndex[n]] << ", " << lctWire[statOneIndex[n]] << endl;
					 cout << "signalPropTime (temp)                    : " << signalPropTime(lctRing[tempVector[n]],lctStation[tempVector[n]],lctStrip[tempVector[n]],lctWire[tempVector[n]]) << endl;
					 cout << "signalPropTime (statOneIndex)            : " << signalPropTime(lctRing[statOneIndex[n]],lctStation[statOneIndex[n]],lctStrip[statOneIndex[n]],lctWire[statOneIndex[n]]) << endl;
					//lctBx[tempVector[i]]
					cout << "Now printing BXs" << endl;				

					for(int b = 0; b < lctBx.size(); b++){
					cout << "BX " << b << " : " << lctBx[b] << endl;
					}
					cout << " lctBx.size(): " <<  lctBx.size() << endl;

					cout << "BX (StatOneIndex)                        : " << lctBx[statOneIndex[n]] << endl;
					cout << "BX (temp)                                : " << lctBx[tempVector[n]] << endl;
					*/
					theHistprop->Fill(lctBx[tempVector[n]] - signalPropTime(lctRing[tempVector[n]],lctStation[tempVector[n]],lctStrip[tempVector[n]],lctWire[tempVector[n]]) );

				}
				else{
					TString gethistprop = TString::Format("BX_Prop_negME_%d%d_%d_run%d",(lctStation[n]),(lctRing[n]),(lctChamber[n]),(runNumber));
					//cout << "the next line is gethistprop" << endl; 
					//cout << gethistprop << endl;
...
for(int i = 0; i < statOneIndex.size(); i++){
	...
	if((TOFendcap2 == 0 && TOFendcap3 == 0) || (TOFendcap2 != 0 && TOFendcap2 == lctEndcap[statOneIndex[i]]) || (TOFendcap3 !=0 && TOFendcap3 == lctEndcap[statOneIndex[i]])){
		//////cout << "LCT In Station 1 (for TOF)" << endl;					
		//////cout << "Checkpoint 1" << endl;
		saveInfo=true;
		ALCT_Bx2=lctBx[statOneIndex[i]];
		Segx1=getx(lctGlobalEta[statOneIndex[i]],lctGlobalPhi[statOneIndex[i]],lctStation[statOneIndex[i]],lctRing[statOneIndex[i]],lctSector[statOneIndex[i]]);
		Segy1=gety(lctGlobalEta[statOneIndex[i]],lctGlobalPhi[statOneIndex[i]],lctStation[statOneIndex[i]],lctRing[statOneIndex[i]],lctSector[statOneIndex[i]]);
		Segz1=getz(lctGlobalEta[statOneIndex[i]],lctGlobalPhi[statOneIndex[i]],lctStation[statOneIndex[i]],lctRing[statOneIndex[i]],lctEndcap[statOneIndex[i]], lctChamber[statOneIndex[i]]);
		TOFchamber = lctChamber[statOneIndex[i]];                        
		TOFendcap = lctEndcap[statOneIndex[i]];
		//////cout << "Checkpoint 2" << endl;
	...
	for(int j = 0; j < statTwoIndex.size(); j++){
		//////cout << "Checkpoint 3" << endl;
		if((TOFendcap == 0 && TOFendcap3 == 0) || (TOFendcap != 0 && TOFendcap == lctEndcap[statTwoIndex[j]]) || (TOFendcap3 !=0 && TOFendcap3 == lctEndcap[statTwoIndex[j]])){
			//////cout << "Checkpoint 4" << endl;
			//////cout << "LCT In Station 2 (for TOF)" << endl;				

			//Make sure that the triggering LCT has 5 <= BX <= 7
			//For LR 228354 the triggering LCT is ME2
			//if(lctBx[statTwoIndex[j]] < 5 || lctBx[statTwoIndex[j]] > 7) cout << "The event should be skipped. ME2 BX: " << lctBx[statTwoIndex[j]] << endl;
			//if((lctBx[statTwoIndex[j]] < 5 || lctBx[statTwoIndex[j]] > 7) && ME2Trigger == 1) continue;
			if(TOFendcap == 1 && statTwoIndexPos.size() == 1 && statThreeIndexPos.size() == 1){
				if((lctBx[statTwoIndexPos[0]] < 5 || lctBx[statTwoIndexPos[0]] > 7 ) && ME2Trigger == 1) continue;
			}else{
				if(TOFendcap == -1 && statTwoIndexNeg.size() == 1 && statThreeIndexNeg.size() == 1){
					if((lctBx[statTwoIndexNeg[0]] < 5 || lctBx[statTwoIndexNeg[0]] > 7 ) && ME2Trigger == 1) continue;
				}else{
					continue;
				}
			}
			...
			hastwo=true;
			Segx2=getx(lctGlobalEta[statTwoIndex[j]],lctGlobalPhi[statTwoIndex[j]],lctStation[statTwoIndex[j]],lctRing[statTwoIndex[j]],lctSector[statTwoIndex[j]]);
			Segy2=gety(lctGlobalEta[statTwoIndex[j]],lctGlobalPhi[statTwoIndex[j]],lctStation[statTwoIndex[j]],lctRing[statTwoIndex[j]],lctSector[statTwoIndex[j]]);
			Segz2=getz(lctGlobalEta[statTwoIndex[j]],lctGlobalPhi[statTwoIndex[j]],lctStation[statTwoIndex[j]],lctRing[statTwoIndex[j]],lctEndcap[statTwoIndex[j]], lctChamber[statTwoIndex[j]]);

			TOFendcap2 = lctEndcap[statTwoIndex[j]];	
			//////cout << "Checkpoint 5" << endl;
		}

		for(int k =0; k < statThreeIndex.size(); k++){
			//////cout << "Checkpoint 6" << endl;
			if((TOFendcap2 == 0 && TOFendcap == 0) || (TOFendcap2 != 0 && TOFendcap2 == lctEndcap[statThreeIndex[k]]) || (TOFendcap !=0 && TOFendcap == lctEndcap[statThreeIndex[k]])){				
				//////cout << "Checkpoint 7" << endl;
				//////cout << "LCT In Station 3 (for TOF)" << endl;				
				//if((lctBx[statThreeIndex[k]] < 5 || lctBx[statThreeIndex[k]] > 7) && ME3Trigger == 1) continue;

				if(TOFendcap == 1 && statTwoIndexPos.size() == 1 && statThreeIndexPos.size() == 1){
					if((lctBx[statThreeIndexPos[0]] < 5 || lctBx[statThreeIndexPos[0]] > 7 ) && ME3Trigger == 1) continue;
				}else{
					if(TOFendcap == -1 && statTwoIndexNeg.size() == 1 && statThreeIndexNeg.size() == 1){
						if((lctBx[statThreeIndexNeg[0]] < 5 || lctBx[statThreeIndexNeg[0]] > 7 ) && ME3Trigger == 1) continue;
					}else{
						continue;
					}
				}
				...
				hasthree=true;
				Segx3=getx(lctGlobalEta[statThreeIndex[k]],lctGlobalPhi[statThreeIndex[k]],lctStation[statThreeIndex[k]],lctRing[statThreeIndex[k]],lctSector[statThreeIndex[k]]);
				Segy3=gety(lctGlobalEta[statThreeIndex[k]],lctGlobalPhi[statThreeIndex[k]],lctStation[statThreeIndex[k]],lctRing[statThreeIndex[k]],lctSector[statThreeIndex[k]]);
				Segz3=getz(lctGlobalEta[statThreeIndex[k]],lctGlobalPhi[statThreeIndex[k]],lctStation[statThreeIndex[k]],lctRing[statThreeIndex[k]],lctEndcap[statThreeIndex[k]], lctChamber[statThreeIndex[k]]);
				TOFendcap3 = lctEndcap[statThreeIndex[k]];			
				//////cout << "Checkpoint 8" << endl;
			}			
			...
			if(saveInfo && hastwo && hasthree) {
				...
				double propCorr = signalPropTime(lctRing[statOneIndex[i]],lctStation[statOneIndex[i]],lctStrip[statOneIndex[i]],lctWire[statOneIndex[i]]);
				if(TOFendcap == -1){
				...
				if(stationInput == 1){
					...
					/*{{{*/
					if(TOFendcap == -1){
						if((lctBx[statTwoIndexNeg[0]] < 5 || lctBx[statTwoIndexNeg[0]] > 7) && ME2Trigger == 1) cout << "There is a problem!" << endl;
						if((lctBx[statThreeIndexNeg[0]] < 5 || lctBx[statThreeIndexNeg[0]] > 7) && ME3Trigger == 1) cout << "There is a problem!" << endl;
						...
						if(deltaY > 100 && deltaY < 1500 && ALCT_Bx2 > 0){
							...
							theHist->Fill(lctStrip[statOneIndex[i]],correctedBX);
						...
						if((lctBx[statTwoIndexPos[0]] < 5 || lctBx[statTwoIndexPos[0]] > 7) && ME2Trigger == 1) cout << "There is a problem!" << endl;
						if((lctBx[statThreeIndexPos[0]] < 5 || lctBx[statThreeIndexPos[0]] > 7) && ME3Trigger == 1) cout << "There is a problem!" << endl;
...
for(int c = 0; c < NeighborTOFcorrectedBX.size(); c++){
	...
	double lowerBXPropCorr = signalPropTime(lctRing[lowerIndex],lctStation[lowerIndex],lctStrip[lowerIndex],lctWire[lowerIndex]);
	double upperBXPropCorr = signalPropTime(lctRing[upperIndex],lctStation[upperIndex],lctStrip[upperIndex],lctWire[upperIndex]);
	...
	if(lowerBXPropCorr < 0){
		...
		cout << "Chamber, Ring, Station, Strip, Wire: " << lctChamber[lowerIndex] << ", " << lctRing[lowerIndex] << ", " << lctStation[lowerIndex] << ", " << lctStrip[lowerIndex] << ", " << lctWire[lowerIndex] << endl;
	...
	//print some info
	if(lctChamber[upperIndex] == 19 || lctChamber[lowerIndex] == 19){				
		if(lctEndcap[lowerIndex] == 1){
			cout << "" << endl;
			cout << "ME" << lctStation[lowerIndex] << lctRing[lowerIndex] << "/" << lctChamber[lowerIndex] << " to ME" << lctStation[upperIndex] << lctRing[upperIndex] << "/" << lctChamber[upperIndex] << " in endcap: " << lctEndcap[lowerIndex] << endl;
			cout << "BX for upper: " << lctBx[upperIndex] << endl;
			cout << "BX for lower: " << lctBx[lowerIndex] << endl; 
			cout << "" << endl;
			cout << "ME2 and ME3 BX: " << lctBx[statTwoIndexPos[0]] << ", " << lctBx[statThreeIndexPos[0]] << endl;
		}

		cout << "" << endl;
	}
	TString gethist = TString::Format("WalkAroundwithTOF_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctStation[upperIndex], lctRing[upperIndex], lctChamber[upperIndex], lctEndcap[lowerIndex]);
	////cout << gethist << endl;					
	TH1F *theHist = ((TH1F *)(WalkAroundwithTOF.FindObject(gethist)));
	theHist->Fill(relBX);
	////cout << "Filled Plot: " << gethist << endl;

	TString gethistA = TString::Format("WalkAroundwithTOF_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctStation[upperIndex], lctRing[upperIndex], lctChamber[upperIndex], lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctEndcap[lowerIndex]);
	////cout << gethistA << endl;			
	TH1F *theHistA = ((TH1F *)(WalkAroundwithTOF.FindObject(gethistA)));
	theHistA->Fill(lowerBX);
	////cout << "Filled Plot: " << gethistA << endl;
	TString gethistB = TString::Format("WalkAroundwithTOF_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctStation[upperIndex], lctRing[upperIndex], lctChamber[upperIndex],lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[upperIndex], lctEndcap[lowerIndex]);
	////cout << gethistB << endl;					
	TH1F *theHistB = ((TH1F *)(WalkAroundwithTOF.FindObject(gethistB)));
	theHistB->Fill(upperBX);

	TString getPropHist1 = TString::Format("WalkAroundwithTOF_And_Prop_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctStation[upperIndex], lctRing[upperIndex], lctChamber[upperIndex], lctEndcap[lowerIndex]);
	TH1F *thePropHist1 = ((TH1F *)(WalkAroundwithTOF_And_Prop.FindObject(getPropHist1)));
	thePropHist1->Fill(relBXProp);

	TString getPropHistA1 = TString::Format("WalkAroundwithTOF_And_Prop_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctStation[upperIndex], lctRing[upperIndex], lctChamber[upperIndex], lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctEndcap[lowerIndex]);
	...
	//********//
	TString getStripHist = TString::Format("NeighborStripTOF_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctStation[upperIndex], lctRing[upperIndex], lctChamber[upperIndex], lctEndcap[lowerIndex]);
	TH2F *theStripHist = ((TH2F *)(NeighborStripTOF.FindObject(getStripHist)));
	theStripHist->Fill((lctStrip[lowerIndex]-lctStrip[upperIndex]),relBX);

	TString getStripHistA = TString::Format("NeighborStripTOF_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctStation[upperIndex], lctRing[upperIndex], lctChamber[upperIndex], lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctEndcap[lowerIndex]);
	TH2F *theStripHistA = ((TH2F *)(NeighborStripTOF.FindObject(getStripHistA)));
	theStripHistA->Fill(lctStrip[lowerIndex],lowerBX);

	TString getStripHistB = TString::Format("NeighborStripTOF_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctStation[upperIndex], lctRing[upperIndex], lctChamber[upperIndex],lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[upperIndex], lctEndcap[lowerIndex]);
	TH2F *theStripHistB = ((TH2F *)(NeighborStripTOF.FindObject(getStripHistB)));
	theStripHistB->Fill(lctStrip[upperIndex],upperBX);

	neighborFillerCounter++;

	TString getPropHist = TString::Format("NeighborStripTOF_And_Prop_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctStation[upperIndex], lctRing[upperIndex], lctChamber[upperIndex], lctEndcap[lowerIndex]);
	TH2F *thePropHist = ((TH2F *)(NeighborStripTOF_And_Prop.FindObject(getPropHist)));
	thePropHist->Fill((lctStrip[lowerIndex]-lctStrip[upperIndex]),relBXProp);

	//cout << "Ring, Station, Strip, Wire: " << lctRing[upperIndex]<< ", " << lctStation[upperIndex]<< ", " << lctStrip[upperIndex]<< ", " << lctWire[upperIndex] << endl;
	//cout << "signalProp: " << signalPropTime(lctRing[upperIndex],lctStation[upperIndex],lctStrip[upperIndex],lctWire[upperIndex]) << endl;
	//cout << " upperBXProp, uppBXProp-upp

	TString getPropHistA = TString::Format("NeighborStripTOF_And_Prop_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctStation[upperIndex], lctRing[upperIndex], lctChamber[upperIndex], lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctEndcap[lowerIndex]);
	TH2F *thePropHistA = ((TH2F *)(NeighborStripTOF_And_Prop.FindObject(getPropHistA)));
	thePropHistA->Fill(lctStrip[lowerIndex],lowerBX-lowerBXPropCorr);

	TString getPropHistB = TString::Format("NeighborStripTOF_And_Prop_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[lowerIndex], lctStation[upperIndex], lctRing[upperIndex], lctChamber[upperIndex],lctStation[lowerIndex], lctRing[lowerIndex], lctChamber[upperIndex], lctEndcap[lowerIndex]);
	TH2F *thePropHistB = ((TH2F *)(NeighborStripTOF_And_Prop.FindObject(getPropHistB)));
	thePropHistB->Fill(lctStrip[upperIndex],upperBX-upperBXPropCorr);
...
for(int i = 0; i<lctBx.size(); i++){
	...
	if(lctStation[i] != 1) continue; 
	if(lctRing[i] != 1) continue;
	numLCTsME11++;
}
...
for(int i=0; i<lctBx.size(); i++){
	for(int j = 0; j < lctBx.size(); j++){
		...
		if(lctEndcap[i] != lctEndcap[j]) continue; //Make sure that the LCTs are in the same endcap					
		if(lctStation[i] != lctStation[j]) continue; //Make sure that the LCTs are in the same station
		if(lctRing[i] != lctRing[j]) continue; // Make sure that the LCTs are in the same ring
		if(lctChamber[i] == lctChamber[j]) continue; //Make sure that the LCTs are NOT in the same chamber	

		//if(csctf_->lctQuality[j] <= 10 || csctf_->lctQuality[i] <= 10) continue; ////3/10/2016 Quality not part of new nTuple
		...
		if(lctStation[i] == 1 || lctRing[i] == 2){ //These chambers go from 1-36
			if((upper == lower+1) || (upper == 36 && lower == 1)){
				...
				int relBX = lctBx[lowerInt] - lctBx[upperInt];
				//Checkpoint
				chiPlotCounter++;

				//////
				double lowerBX = lctBx[lowerInt];
				double upperBX = lctBx[upperInt];
				if(  PileUp50ns == 1 ){
					if(upperBX > 7 || upperBX < 5) continue;					
					if(lowerBX > 7 || lowerBX < 5) continue;
				}
				...
				*/
				double lowerBXPropCorr = signalPropTime(lctRing[lowerInt],lctStation[lowerInt],lctStrip[lowerInt],lctWire[lowerInt]);
				double upperBXPropCorr = signalPropTime(lctRing[upperInt],lctStation[upperInt],lctStrip[upperInt],lctWire[upperInt]);
				...
				if(upper == lower+1){
					TString gethist = TString::Format(	           "WalkAround_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",lctStation[i], lctRing[i], lower, lctStation[j], lctRing[j], upper, lctEndcap[i]); 
					TH1F *theHist = ((TH1F *)(WalkAround.FindObject(gethist)));
					//////cout << "gethist, theHist, relBX: " << gethist << ", " << theHist << ", " << relBX << endl;
					theHist->Fill(relBX);
					TString gethistA = TString::Format("WalkAround_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[i], lctRing[i], lower, lctStation[j], lctRing[j], upper, lctStation[i], lctRing[i], lower, lctEndcap[i]);
					TH1F *theHistA = ((TH1F *)(WalkAround.FindObject(gethistA)));
					theHistA->Fill(lctBx[lowerInt]);
					TString gethistB = TString::Format("WalkAround_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[i], lctRing[i], lower, lctStation[j], lctRing[j], upper, lctStation[i], lctRing[i], upper, lctEndcap[i]);
					TH1F *theHistB = ((TH1F *)(WalkAround.FindObject(gethistB)));
					theHistB->Fill(lctBx[upperInt]);


					///prop

					TString getPropHist1 = TString::Format(                 "WalkAroundProp_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",lctStation[i], lctRing[i], lower, lctStation[j], lctRing[j], upper, lctEndcap[i]);
					TH1F *thePropHist1 = ((TH1F *)(WalkAroundProp.FindObject(getPropHist1)));
					thePropHist1->Fill(relBXProp);

					TString getPropHistA1 = TString::Format("WalkAroundProp_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[i], lctRing[i], lower, lctStation[j], lctRing[j], upper, lctStation[i], lctRing[i], lower, lctEndcap[i]);
					TH1F *thePropHistA1 = ((TH1F *)(WalkAroundProp.FindObject(getPropHistA1)));
					thePropHistA1->Fill(lowprop);
					TString getPropHistB1 = TString::Format("WalkAroundProp_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[i], lctRing[i], lower, lctStation[j], lctRing[j], upper, lctStation[i], lctRing[i], upper, lctEndcap[i]);
					TH1F *thePropHistB1 = ((TH1F *)(WalkAroundProp.FindObject(getPropHistB1)));
					thePropHistB1->Fill(highprop);

				}else{
					TString gethist = TString::Format(                 "WalkAround_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",lctStation[i], lctRing[i], upper, lctStation[j], lctRing[j], lower, lctEndcap[i]); 
					TH1F *theHist = ((TH1F *)(WalkAround.FindObject(gethist)));
					theHist->Fill(relBX);

					TString gethistA = TString::Format("WalkAround_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[i], lctRing[i], upper, lctStation[j], lctRing[j], lower, lctStation[i], lctRing[i], lower, lctEndcap[i]);
					TH1F *theHistA = ((TH1F *)(WalkAround.FindObject(gethistA)));
					theHistA->Fill(lctBx[lowerInt]);
					TString gethistB = TString::Format("WalkAround_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[i], lctRing[i], upper, lctStation[j], lctRing[j], lower, lctStation[i], lctRing[i], upper, lctEndcap[i]);
					TH1F *theHistB = ((TH1F *)(WalkAround.FindObject(gethistB)));
					theHistB->Fill(lctBx[upperInt]);

					///prop

					TString getPropHist1 = TString::Format(                 "WalkAroundProp_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",lctStation[i], lctRing[i], upper, lctStation[j], lctRing[j], lower, lctEndcap[i]);
					TH1F *thePropHist1 = ((TH1F *)(WalkAroundProp.FindObject(getPropHist1)));
					thePropHist1->Fill(relBXProp);
					TString getPropHistA1 = TString::Format("WalkAroundProp_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[i], lctRing[i], upper, lctStation[j], lctRing[j], lower, lctStation[i], lctRing[i], lower, lctEndcap[i]);
					TH1F *thePropHistA1 = ((TH1F *)(WalkAroundProp.FindObject(getPropHistA1)));
					thePropHistA1->Fill(lowprop);
					TString getPropHistB1 = TString::Format("WalkAroundProp_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",lctStation[i], lctRing[i], upper, lctStation[j], lctRing[j], lower, lctStation[i], lctRing[i], upper, lctEndcap[i]);
					TH1F *thePropHistB1 = ((TH1F *)(WalkAroundProp.FindObject(getPropHistB1)));
					thePropHistB1->Fill(highprop);
...
if(lctEndcap[i] != lctEndcap[j]) continue; //Make sure that the LCTs are in the same endcap
if(lctStation[i] != lctStation[j]) continue; //Make sure that the LCTs are in the same station
if(lctRing[i] == lctRing[j]) continue; // Make sure that the LCTs are NOT the same ring
int outerRing = 0;
int innerRing = 0;
int innerChamber = 0;
int outerChamber = 0;
...
if(lctRing[i] > lctRing[j]){
outerRing = lctRing[i];
innerRing = lctRing[j];

innerChamber = lctChamber[j];
outerChamber = lctChamber[i];
}else{
outerRing = lctRing[j];
innerRing = lctRing[i];

outerChamber = lctChamber[j];
innerChamber = lctChamber[i];					
}
if(lctStation[i] == 1){
...
int relBX = lctBx[i] - lctBx[j];
TString gethist = TString::Format("WalkAround_Stations_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",1, innerRing, lctChamber[i], 1, outerRing, lctChamber[i], lctEndcap[i]);	
...
TString gethist = TString::Format("WalkAround_Stations_RelBX_ME%d%d_%d_to_ME%d%d_%d_or_%d_Endcap_%d",lctStation[i], innerRing, innerChamber, lctStation[j], outerRing, TwoChamber-1, TwoChamber, lctEndcap[i]);
TH1F *theHist = ((TH1F *)(WalkAround2.FindObject(gethist)));
////cout << gethist << endl;
theHist->Fill(relBX);
}
...
/*
   ...
   if(vector_contains(duplicateList, i) == true) continue; //Make sure not to count the duplicates
   if(vector_contains(duplicateList, j) == true) continue; //Make sure not to count the duplicates
   if(lctStation[i] == lctStation[j]) continue; //different stations
   if(lctSector[i] != lctSector[j]) continue; //same sector
   ...
   if(lctStation[lowInt]+1 != lctStation[highInt]) continue; //make sure that the stations are neighboring
   usedLCTpairs_Disks.push_back(LCTpair);                                                       
   int relBX = lctBx[i] - lctBx[j];

   TString gethist = TString::Format("WalkAround_Disks_RelBX_Sector_%d_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",lctSector[i],lctStation[lowInt],lctRing[lowInt],lctChamber[lowInt],lctStation[highInt],lctRing[highInt],lctChamber[highInt],lctEndcap[i]);
