...
void MakePlots(TString filename, int runNumber, int station, int ring, int numChambersInRing, int EndcapNeg1Or2){
	//Load File
...
endcap = -1;
	...
	vector<double>  clbg_x; //clbg=EMTF LCT BX Graph
	...
	vector<double> all_x;
	vector<double> all_y;
	vector<double> all_ex;
	vector<double> all_ey;
	...
	//quick loop to plot ALL chambers for an endcap
	for(int i = 1; i < 37; i++){
		TString bxHist = TString("BX_")+cap+TString::Format("ME_%d%d_%d_run%d",1,1,i,runNumber);

		all_x.push_back(i);
		all_y.push_back((getHist(tfile,bxHist)->GetMean()));
		all_ex.push_back( (0.5));
		all_ey.push_back(  (getHist(tfile, bxHist)->GetMeanError()));


                TString bxHist = TString("BX_")+cap+TString::Format("ME_%d%d_%d_run%d",1,2,i,runNumber);

                all_x.push_back(i+36);
                all_y.push_back((getHist(tfile,bxHist)->GetMean()));
                all_ex.push_back( (0.5));
                all_ey.push_back(  (getHist(tfile, bxHist)->GetMeanError()));

                TString bxHist = TString("BX_")+cap+TString::Format("ME_%d%d_%d_run%d",1,3,i,runNumber);

                all_x.push_back(i+72);
                all_y.push_back((getHist(tfile,bxHist)->GetMean()));
                all_ex.push_back( (0.5));
                all_ey.push_back(  (getHist(tfile, bxHist)->GetMeanError()));

                TString bxHist = TString("BX_")+cap+TString::Format("ME_%d%d_%d_run%d",2,2,i,runNumber);

                all_x.push_back(i+126);
                all_y.push_back((getHist(tfile,bxHist)->GetMean()));
                all_ex.push_back( (0.5));
                all_ey.push_back(  (getHist(tfile, bxHist)->GetMeanError()));

                TString bxHist = TString("BX_")+cap+TString::Format("ME_%d%d_%d_run%d",3,2,i,runNumber);

                all_x.push_back(i+180);
                all_y.push_back((getHist(tfile,bxHist)->GetMean()));
                all_ex.push_back( (0.5));
                all_ey.push_back(  (getHist(tfile, bxHist)->GetMeanError()));

                TString bxHist = TString("BX_")+cap+TString::Format("ME_%d%d_%d_run%d",4,2,i,runNumber);

                all_x.push_back(i+234);
                all_y.push_back((getHist(tfile,bxHist)->GetMean()));
                all_ex.push_back( (0.5));
                all_ey.push_back(  (getHist(tfile, bxHist)->GetMeanError()));
...
for(int i = 1; i < 19; i++){
                TString bxHist = TString("BX_")+cap+TString::Format("ME_%d%d_%d_run%d",2,1,i,runNumber);

                all_x.push_back(i+108);
                all_y.push_back((getHist(tfile,bxHist)->GetMean()));
                all_ex.push_back( (0.5));
                all_ey.push_back(  (getHist(tfile, bxHist)->GetMeanError()));


                TString bxHist = TString("BX_")+cap+TString::Format("ME_%d%d_%d_run%d",3,1,i,runNumber);

                all_x.push_back(i+162);
                all_y.push_back((getHist(tfile,bxHist)->GetMean()));
                all_ex.push_back( (0.5));
                all_ey.push_back(  (getHist(tfile, bxHist)->GetMeanError()));


                TString bxHist = TString("BX_")+cap+TString::Format("ME_%d%d_%d_run%d",4,1,i,runNumber);

                all_x.push_back(i+216);
                all_y.push_back((getHist(tfile,bxHist)->GetMean()));
                all_ex.push_back( (0.5));
                all_ey.push_back(  (getHist(tfile, bxHist)->GetMeanError()));
}
	...
	for(int x = 1; x < numChambersInRing+1; x++){
		...
		TString waRelBX = TString::Format("WalkAround_Rings_RelBX_ME%d%d_%d_to_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,EndcapNeg1Or2);
		...
		TString waIndBX_low = TString::Format("WalkAround_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,station,ring,lowChamber,EndcapNeg1Or2);
		TString waIndBX_high = TString::Format("WalkAround_Rings_ME%d%d_%d_and_ME%d%d_%d_Overlap_BX_ME%d%d_%d_Endcap_%d",station,ring,lowChamber,station,ring,highChamber,station,ring,highChamber,EndcapNeg1Or2);
...
double* array_all_x  = &all_x[0];
double* array_all_y  = &all_y[0];
double* array_all_ex = &all_ex[0];
double* array_all_ey = &all_ey[0];
...
TGraphErrors* all_graph = new TGraphErrors(270, array_all_x, array_all_y, array_all_ex, array_all_ey);
	...
	graph->SetTitle(TString("EMTF LCT BX ME")+sign+Form("%d%d",station,ring));
	...
	cout << "" << endl;
	cout << "Fit information from the EMTF LCT BX plot: " << endl;
	graph->Fit("pol0","F"); //The F option uses the minuit fitter.  Adding Q ("FQ") makes the fit not print.
	cout << "" << endl;

        all_graph->SetTitle("EMTF LCT BX for ME" + sign);
        all_graph->SetMarkerColor(2);
        all_graph->SetMarkerStyle(21);
        all_graph->SetMarkerSize(1);
        all_graph->GetXaxis()->SetTitle("Chamber Number");
        all_graph->GetYaxis()->SetTitle("BX (25ns)");
        TCanvas* myAllcanvas = new TCanvas();
        myAllcanvas->SetGridy();
        TAxis *axisall = all_graph->GetYaxis();
        axisall->SetRangeUser(4,8);
        all_graph->Draw("AP");
gStyle->SetOptFit(0001);
        cout << "" << endl;
        cout << "Fit information from the EMTF LCT BX plot: " << endl;
        all_graph->Fit("pol0","F"); //The F option uses the minuit fitter.  Adding Q ("FQ") makes the fit not print.

	TLine *me11 = new TLine(36, 4, 36, 8);
	TLine *me12 = new TLine(72, 4, 72, 8);
	TLine *me13 = new TLine(108, 4, 108, 8);
	TLine *me21 = new TLine(126, 4, 126, 8);
	TLine *me22 = new TLine(162, 4, 162, 8);
	TLine *me31 = new TLine(180, 4, 180, 8);
	TLine *me32 = new TLine(216, 4, 216, 8);
	TLine *me41 = new TLine(234, 4, 234, 8);
        
me11->SetLineColor(kBlue);  
me12->SetLineColor(kBlue);
me13->SetLineColor(kBlue);
me21->SetLineColor(kBlue);
me22->SetLineColor(kBlue);
me31->SetLineColor(kBlue);
me32->SetLineColor(kBlue);
me41->SetLineColor(kBlue);
...
me41->Draw("same");
	...
	tof_bx_graph->SetTitle(TString("EMTF LCT BX With TOF Corrections ME")+sign+Form("%d%d",station,ring));
	...
	gStyle->SetOptFit(0001);
	cout << "" << endl;
	cout << "Fit information from the EMTF LCT BX With TOF Corrections plot: " << endl;
	...
	myAllcanvas->SaveAs("BX_By_All_Chambers.pdf");
